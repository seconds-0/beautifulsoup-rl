...
def test_b2_connection() -> TestResult:
    ...
    try:
        # Authorize B2
        proc = subprocess.run(
            ["b2", "authorize-account", key_id, app_key], capture_output=True, text=True, timeout=30
        )
        ...
        # List buckets
        proc = subprocess.run(["b2", "ls", "--json"], capture_output=True, text=True, timeout=30)
        if proc.returncode != 0:
        ...
        proc = subprocess.run(
            ["b2", "ls", "beautifulsoup-rl"], capture_output=True, text=True, timeout=30
        )
        if proc.returncode != 0:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            proc = subprocess.run(
                [
                    ...
                    "beautifulsoup-rl",
                    str(test_file),
                    "test/resilient_training_test.txt",
                ],
                capture_output=True,
                text=True,
                timeout=60,
            ...
            if downloaded_content != test_content:
                result.message = (
                    f"Content mismatch: expected {test_content!r}, got {downloaded_content!r}"
                )
                return result
...
def test_wandb_connection() -> TestResult:
    ...
    try:
        ...
        runs = api.runs("seconds-0-domus-magna-inc/beautiful-soup-env", per_page=1)
...
def test_wandb_monitor() -> TestResult:
    ...
    try:
        proc = subprocess.run(
            [sys.executable, str(script_path), "--run-id", "bs4-qwen3-8b", "--json"],
            capture_output=True,
            text=True,
            timeout=60,
    ...
    try:
        ...
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                f'import sys; sys.path.insert(0, "{script_path.parent}"); import training_controller',
            ],
            capture_output=True,
            text=True,
            timeout=10,
...
def main():
    ...
    parser.add_argument(
        "--test",
        choices=["b2", "wandb", "vast", "scripts", "all"],
        default="all",
